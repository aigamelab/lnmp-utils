...
show_help(){
	...
	echo ""
	echo "component list:"
	echo "web:     openresty(nginx+lua)"
	echo "db:      mariadb"
	echo "lang:    php(8.4/8.5)"
	echo "nosql:   redis memcached"
	echo "dfs:     lsyncd(rsync)"
	echo "runtime: node"
	...
	echo "                     ./install.sh -c mariadb php redis openresty node ..."
	echo "-m|--mode=xxx        Install modules"
	echo "-o|--option          Options for installing components"
	echo "                     ./install.sh -c openresty -o proxy"
